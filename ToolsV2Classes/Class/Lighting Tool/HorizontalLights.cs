...
namespace ToolsV2Classes
{
    ...
    public class HorizontalLights : IExternalCommand
    {
            ...
            double mountingHeight, Transform tForm, double roomAngle)
        {
            ...
            if (roomBox != null)
            {
                ...
                int nMin = (int)Math.Ceiling(((L + 2400) / 3600) - 1);
                ...
                for (int j = 1; j <= nMin; j++)
                {
                    ...
                    {
                        XYZ tempPoint1 = new XYZ((xMinVal + yWall + (j - 1) * y) / 304.8, (yMinVal + x + (i - 1) * 2 * x) / 304.8, zVal / 304.8);
                        if (tForm != null)
                        {
