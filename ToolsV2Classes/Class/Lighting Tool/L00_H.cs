...
namespace ToolsV2Classes
{
    ...
    public class L00_H : IExternalCommand
    {
        ...
        public void ModelOpsLights(FamilySymbol lightSymbol, BoundingBoxXYZ roomBox, Room room1, Document doc, Element roomLevel, double mountingHeight)
        {
            ...
            if (roomBox != null)
            {
                ...
                for (int j = 1; j <= nMin; j++)
                {
                    ...
                    {
                        XYZ tempPoint1 = new XYZ((xMinVal + y + (j - 1) * 3000) / 304.8, (yMinVal + x + (i - 1) * 1200) / 304.8, zVal / 304.8);
                        XYZ tempPoint2 = new XYZ((xMinVal + y + (j - 1) * 3000) / 304.8, (yMinVal + x + (i - 1) * 1200) / 304.8, (zVal + 1000) / 304.8);
